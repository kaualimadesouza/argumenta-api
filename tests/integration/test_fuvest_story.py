"""Issue #16: a história FUVEST "Sinal Fechado" completa e jogável."""
...
from argumenta.adapters.db.seed.fuvest_traffic import STORY_SLUG, seed_fuvest_traffic
...
class TestStoryShape:
    def test_the_seed_is_idempotent(self, db_engine: Engine) -> None:
        with Session(db_engine) as session:
            assert seed_fuvest_traffic(session) is True
            assert seed_fuvest_traffic(session) is False
            session.commit()

        with Session(db_engine) as session:
            assert len(session.scalars(select(Story).where(Story.slug == STORY_SLUG)).all()) == 1
    ...
    def test_the_story_carries_the_real_exam_theme(self, seeded: None, db_engine: Engine) -> None:
        ...
        assert (theme.exam, theme.year) == (Exam.FUVEST, 2021)
        assert "fora de ordem" in theme.title
        assert "O mundo contemporâneo" in theme.statement

    def test_the_story_sits_after_the_enem_with_its_own_ruler(
        self, seeded: None, db_engine: Engine
    ) -> None:
        with Session(db_engine) as session:
            story = _story(session)

        assert (story.position, story.is_tutorial) == (3, False)
        assert (story.dimension_floor, story.min_average) == (55, 65)
...
class TestPlayable:
    def test_the_track_offers_the_story_after_the_enem(
        ...
        assert list(stories) == ["o-gremio", "cuidado-invisivel", "sinal-fechado"]
        assert stories["sinal-fechado"]["state"] == "locked"
        assert stories["sinal-fechado"]["chapters_total"] == 5
    ...
    def test_the_whole_story_can_be_played_to_the_end(
        ...
        assert len(titles) == 13, titles
        assert titles[8:] == [
            "A pressa no trânsito",
            "A nostalgia do motorista",
            "A ordem do guarda",
            "A sobrevivência no asfalto",
            "A banca da FUVEST",
        ]
        track = client.get("/track").json()
        states = [story["state"] for story in track["stories"]]
        assert states == [
            "completed",
            "completed",
            "completed",
